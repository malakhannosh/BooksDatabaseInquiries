"""Comp421 support module with tools for downloading assignments, lecture notes, and exams
...
def check_for_loops(function):
    class visitor(ast.NodeVisitor):
        '''Abstract syntax tree visitor'''
        def visit_For(self, node):
            raise UserWarning('For not allowed')
        def visit_comprehension(self, node):
            raise UserWarning('List comprehension not allowed')
        def visit_While(self, node):
            raise UserWarning('While not allowed')
